...
for i = 1:U
    for n = 1:N
        for m = 1:M
            if Psi(i, n, m) > 1
                recordPowerOffload(i, n, m) = B*N0/g(i, n, m)*(exp(J*x(Psi(i, n, m), n, m))-exp(J*x(Psi(i, n, m)-1, n, m)));
            elseif Psi(i, n, m) == 1
                recordPowerOffload(i, n, m) = B*N0/g(i, n, m)*(exp(J*x(Psi(i), n, m))-1);
            end
        end
    end
end
...
figure(2)
for i = 1:U
        stem3(i*ones(N, 1), (1:N), recordPowerOffload(i, :));
        hold on
end
xlabel('User Index')
ylabel('Time Slot')
